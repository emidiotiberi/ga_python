...
#    x2=np.linspace(x2min,x2max,R)
    ...
    for i in range(np.size(pop,0)):
        val =  500**-1;
        for k in range(25):
            val = val + (k+1+((X[i,0])-a[0,k])**6+((X[i,1])-a[1,k])**6)**(-1)
...
def select_mating_pool(pop, fitness, num_parents):
    ...
    parents = np.empty((num_parents, pop.shape[1]),dtype=int)
...
def crossover2(parents,n_cross_point, offspring_size):
    offspring_cross = np.empty(offspring_size)
    # The point at which crossover takes place between two parents. Usually it is at the center.
#    crossover_point = np.uint8(offspring_size[1]/2)
    ...
    for k in range(offspring_size[0]):
        ...
        # Index of the second parent to mate.
        parent2_idx = (k+1)%parents.shape[0]
        offspring_cross[k,:] = parents[parent1_idx,:]&parents[parent2_idx,:]
...
def mutation(offspring_mutation_num,new_population,n_mut_point):
    ...
    for idx in range(offspring_mutation.shape[0]):
        ...
        mutationpoints = np.random.permutation(offspring_mutation.shape[1]-1)
        mutationpoints = np.sort(mutationpoints[:n_mut_point])
        for idx2 in range(n_mut_point):
            offspring_mutation[idx, mutationpoints[idx2]] = offspring_mutation[idx, mutationpoints[idx2]]^1
